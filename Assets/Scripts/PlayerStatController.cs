...
public class PlayerStatController : MonoBehaviour
{
    ...
    public void DropMagnet(Vector3 position, int value)
    {
        MagnetPickup newMagnet = Instantiate(magnet, position + new Vector3(.2f, .1f, 0f), Quaternion.identity);
        newMagnet.magnetAmount = value;
        newMagnet.gameObject.SetActive(true);
    }
    ...
    public void AddMagnet(int value)
    {
        ...
        UpdateDisplay();
        PlayerController.instance.pickupRange = pickupRange[pickupRangeLevel].value;
        this.CallWithDelay(RemoveMagnet,delay);
    }

    void RemoveMagnet()
    {
        pickupRangeLevel = 0;
        PlayerController.instance.pickupRange = 1f;
    }
